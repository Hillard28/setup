...
function setTheme(theme: string) {
    ...
    execAsync(`~/.config/themes/setTheme.sh ${theme}`)
        .finally(() => {
            buttonsEnabled = true
        })
}
...
function updateFiles() {
    execAsync(["bash", "-c", `ls ~/.config/themes/${ThemeDetails[selectedTheme.get()].name}/wallpaper`])
        .catch((error) => {
            print(error)
        })
        .then((value) => {
...
function setWallpaper(path: string) {
    execAsync(["bash", "-c", `~/.config/themes/setWallpaper.sh ${path}`])
        .catch((error) => {
            print(error)
        })
}
    ...
    return <box
        ...
        {files((filesList) => {
            ...
            return filesList[column].map((file) => {
                const path = `~/.config/themes/${ThemeDetails[selectedTheme.get()].name}/wallpaper/${file}`
